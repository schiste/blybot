...
def _group_scope(scope: Scope) -> Scope:
    """The group-default scope (empty thread) ``scope`` inherits from."""
    return scope if not scope.thread else replace(scope, thread="")
...
class ChannelSettings:
    ...
    has_token: bool
    events_enabled: bool
    # The scope whose profile provided the repo/token binding — the
    # token must be fetched from THIS scope, not the calling topic (a
...
class ChannelDirectory:
    ...
    async def resolve(self, scope: Scope) -> ChannelSettings:
        """Resolve a scope's settings; never raises.

        Three tiers: a topic's own profile overrides the group default
        (the empty-thread scope) overrides the operator env defaults. The
        page falls back per-field; the repo/token/events resolve as a
        *unit* from the first tier that binds a repo (so a topic's token
        is used only for the topic's own repo); consent is group-level
        only. A storage outage degrades to env defaults so /log keeps
        working.
        """
        group_scope = _group_scope(scope)
        group: GroupProfile | None = None
        topic: GroupProfile | None = None
        degraded = False
        if self.store is not None:
            try:
                group = await self.store.get(group_scope)
                topic = await self.store.get(scope) if scope.thread else group
            except StorageError:
                degraded = True
        ...
        return ChannelSettings(
            ...
            repo_scope=binder.scope if binder else group_scope,
            customized=bool((topic and topic is not group) or group),
            degraded=degraded,
        )

    async def set_log_page(self, scope: Scope, title: str) -> str:
        """Point this scope's ``/log`` at a page.
        ...
        await self._update(scope, log_page=page)
        return page
    ...
    async def set_consent(self, scope: Scope, mode: ConsentMode) -> None:
        """Set the group's consent policy for ``/log`` (group-wide, thread 0)."""
        await self._update(_group_scope(scope), consent_mode=mode)

    async def set_repo(self, scope: Scope, repo: str) -> None:
        """Bind this scope to a GitHub repository (``owner/name``)."""
        await self._update(scope, repo=repo)

    async def set_events(self, scope: Scope, *, enabled: bool) -> None:
        """Switch this scope's rule-driven notifications on or off."""
        await self._update(scope, events_enabled=enabled)

    async def set_capture(self, scope: Scope, *, enabled: bool) -> None:
        """Switch this scope's message capture on or off (v3 plan §2.7)."""
        await self._update(scope, capture_enabled=enabled)

    async def set_llm(self, scope: Scope, settings: LlmSettings | None) -> None:
        """Store (or with ``None`` clear) this scope's LLM settings (v3 §2.4)."""
        await self._update(scope, llm=settings)

    async def set_subscribe_code(self, scope: Scope, code: str | None) -> None:
    ...
    async def enable_events(self, scope: Scope, seed_rules: tuple[Rule, ...]) -> bool:
        ...
        current = await self._profile(store, scope)
    ...
    async def add_rule(self, scope: Scope, rule: Rule) -> tuple[Rule, ...]:
        """Append a composable event rule to this scope.
    ...
    async def remove_rule(self, scope: Scope, rule_id: str) -> bool:
        """Drop the rule with ``rule_id``; ``False`` if no such rule exists."""
        store = self._require_store()
        current = await store.get(scope)
        if current is None:
            return False
    ...
    async def clear_rules(self, scope: Scope) -> int:
    ...
    async def list_rules(self, scope: Scope) -> tuple[Rule, ...]:
        """Return this scope's stored rules (empty tuple if none)."""
        return (await self.profile_of(scope)).rules

    async def migrate(self, old: Scope, new: Scope) -> None:
        """Carry every topic's profile across a group→supergroup chat-id change."""
        if self.store is not None:
            await self.store.migrate(old, new)

    async def reset(self, scope: Scope) -> None:
        """Forget this scope's profile (token and cursor included)."""
        store = self._require_store()
        await store.delete(scope)

    async def profile_of(self, scope: Scope) -> GroupProfile:
        """Return the stored profile (or an empty one) for /settings display."""
        return await self._profile(self._require_store(), scope)

    async def _update(self, scope: Scope, **changes: Any) -> None:
        store = self._require_store()
        current = await self._profile(store, scope)
        await store.upsert(replace(current, **changes))

    @staticmethod
    async def _profile(store: ProfileStore, scope: Scope) -> GroupProfile:
        """The stored profile for a scope, or a fresh empty one."""
        return await store.get(scope) or GroupProfile(scope=scope)
