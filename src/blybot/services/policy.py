...
class GroupPolicy:
    ...
    exactly those channels. Entries are compared as **strings**, matching
    a :class:`Scope`'s opaque ``channel`` part: Telegram and Discord ids
    are numeric, but an IRC channel is ``#name``, and int-coercing it
    raised (issue #19). Supergroup migration (spec 8) rewrites the
    ...
    def is_allowed(self, scope: Scope) -> bool:
        ...
        return not self.allowed or scope.channel in self.allowed

    def migrate(self, old: Scope, new: Scope) -> bool:
        """Rewrite ``old``'s channel to ``new``'s; return whether it applied."""
        if old.channel in self.allowed:
            self.allowed.discard(old.channel)
            self.allowed.add(new.channel)
            return True
        return False
